...
| read float32 (streaming) | Yes |  | assume the number fit in current buffer, it turns out the number end not found, fallback to slow path, read byte by byte then Float.valueOf |
| read float64 | Yes |  |
